...
export const getAllSERVICEDETAils = (q,limit,page) => async (dispatch) => {
//console.log('page1234',query1,typeof show);
const token=localStorage.getItem("token")
//const limit=Number(show)
  try {
    dispatch({ type: GET_ALL_SERVICEDETAILS_REQUEST });
   
    
    const res=  await fetch(`http://localhost:8080/detail/getall?query=${q}&limit=${limit}&page=${page}`, {
...
console.log('fgh12345',data);
    dispatch({ type: GET_ALL_SERVICEDETAILS_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: GET_ALL_SERVICEDETAILS_FAILURE,
      payload:error
    });
  }
};
...
export const deleteservicedetails = (id) => async (dispatch) => {
  try {
    dispatch({ type: DELETE_SERVICEDETAILS_REQUEST });
    ...
    dispatch({ type: DELETE_SERVICEDETAILS_SUCCESS, payload: id });
  } catch (error) {
    dispatch({
      type: DELETE_SERVICEDETAILS_FAILURE,
    });
  }
};
...
export const addservicedetaildata = (data) => async (dispatch) => {
  console.log('datajhbhb',data);
    try {
      dispatch({ type: ADD_SERVICEDETAILS_REQUEST });
      ...
      //     })
          ...
          // try {
             const response = await axios.post('http://localhost:8080/detail/upload', data, {
             headers: {
                'Content-Type': 'multipart/form-data',
              },
           });
          ...
          //   setMessage(response.data);
                dispatch({ type: ADD_SERVICEDETAILS_SUCCESS, payload: response.data });
      ...
      dispatch({
        type: ADD_SERVICEDETAILS_FAILURE,
      });
    }
  };
