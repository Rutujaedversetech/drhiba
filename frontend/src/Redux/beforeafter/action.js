...
export const getAllbefore = () => async (dispatch) => {
//console.log('page1234',query1,typeof show);
const token=localStorage.getItem("token")
//const limit=Number(show)
  try {
    dispatch({ type: GET_ALL_BEFOREAFTER_REQUEST });
   
    
    const res=  await fetch(`http://localhost:8080/before/getall`, {
...
console.log('fgh12345',data);
    dispatch({ type: GET_ALL_BEFOREAFTER_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: GET_ALL_BEFOREAFTER_FAILURE,
      payload:error
    });
  }
};
...
export const deletebeforeimage = (id) => async (dispatch) => {
  try {
    dispatch({ type: DELETE_BEFOREAFTER_REQUEST });

    await fetch(`http://localhost:8080/before/${id}`, {
      method: "DELETE",
      headers: {
       // token: localStorage.getItem("token"),
        "Content-Type": "application/json",
    ...
    dispatch({ type: DELETE_BEFOREAFTER_SUCCESS, payload: id });
  } catch (error) {
    dispatch({
      type: DELETE_BEFOREAFTER_FAILURE,
    });
  }
};
...
export const addbeforeShow = (data) => async (dispatch) => {
  console.log('datajhbhb',data);
    try {
      dispatch({ type: ADD_BEFOREAFTER_REQUEST });
      ...
      //     })
          ...
          // try {
             const response = await axios.post('http://localhost:8080/before/upload', data, {
             headers: {
                'Content-Type': 'multipart/form-data',
              },
           });
          ...
          //   setMessage(response.data);
                dispatch({ type: ADD_BEFOREAFTER_SUCCESS, payload: response.data });
