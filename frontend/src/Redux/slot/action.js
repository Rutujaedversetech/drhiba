...
export const getAllSlot = (q,limit,page) => async (dispatch) => {
//console.log('page1234',query1,typeof show);
const token=localStorage.getItem("token")
//const limit=Number(show)
  try {
    dispatch({ type: GET_ALL_SLOT_REQUEST });
   
    
    const res=  await fetch(`http://localhost:8080/slot/getall?query=${q}&limit=${limit}&page=${page}`, {
...
console.log('fgh12345',data);
    dispatch({ type: GET_ALL_SLOT_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: GET_ALL_SLOT_FAILURE,
      payload:error
    });
  }
};
...
export const deleteslotdata = (id) => async (dispatch) => {
  try {
    dispatch({ type: DELETE_SLOT_REQUEST });

    await fetch(`http://localhost:8080/slot/${id}`, {
      method: "DELETE",
      headers: {
       // token: localStorage.getItem("token"),
        "Content-Type": "application/json",
    ...
    dispatch({ type: DELETE_SLOT_SUCCESS, payload: id });
  } catch (error) {
    dispatch({
...
export const addslotetails = (data) => async (dispatch) => {
  console.log('datajhbhb',data);
    try {
      dispatch({ type: ADD_SLOT_REQUEST });
      ...
      axios.post('http://localhost:8080/slot/upload',
          data,
          // {
          ...
          .then((res) => {
              console.log(res.data)
              dispatch({ type: ADD_SLOT_SUCCESS, payload: res.data });
