...
//const limit=Number(show)
  try {
    ...
    const res=  await fetch(`http://localhost:8080/service/getall`, {
...
console.log('fgh12345',data);
    dispatch({ type: GET_ALL_SERVICE_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
...
export const deleteServicedata = (id) => async (dispatch) => {
  try {
    ...
    await fetch(`http://localhost:8080/service/${id}`, {
      method: "DELETE",
      headers: {
       // token: localStorage.getItem("token"),
        "Content-Type": "application/json",
...
export const addServicedetails = (data) => async (dispatch) => {
  console.log('datajhbhb',data);
    try {
      ...
      axios.post('http://localhost:8080/service/upload',
          data,
          // {
