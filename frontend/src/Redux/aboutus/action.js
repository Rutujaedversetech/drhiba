...
//const limit=Number(show)
  try {
    dispatch({ type: GET_ALL_ABOUT_REQUEST });
   
    
    const res=  await fetch(`http://localhost:8080/about/getall`, {
...
console.log('fgh12345',data);
    dispatch({ type: GET_ALL_ABOUT_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: GET_ALL_ABOUT_FAILURE,
      payload:error
    });
  }
};
...
export const deleteaboutdata = (id) => async (dispatch) => {
  try {
    dispatch({ type: DELETE_ABOUT_REQUEST });

    await fetch(`http://localhost:8080/about/${id}`, {
      method: "DELETE",
      headers: {
       // token: localStorage.getItem("token"),
        "Content-Type": "application/json",
    ...
    dispatch({ type: DELETE_ABOUT_SUCCESS, payload: id });
  } catch (error) {
    dispatch({
      type: DELETE_ABOUT_FAILURE,
    });
  }
};
...
export const addaboutetails = (data) => async (dispatch) => {
  console.log('datajhbhb',data);
    try {
      dispatch({ type: ADD_ABOUT_REQUEST});
      ...
      axios.post('http://localhost:8080/about/upload',
          data,
          // {
