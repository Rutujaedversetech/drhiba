...
import {
  ...
  ADD_SLOT_SUCCESS,
  ADD_SLOT_FAILURE,
  GET_ALL_SLOTHOLIDAY_REQUEST,
  GET_ALL_SLOTHOLIDAY_SUCCESS,
  GET_ALL_SLOTHOLIDAY_FAILURE,
  DELETE_SLOTHOLIDAY_REQUEST,
  DELETE_SLOTHOLIDAY_SUCCESS,
  DELETE_SLOTHOLIDAY_FAILURE,
  ADD_SLOTHOLIDAY_REQUEST,
  ADD_SLOTHOLIDAY_SUCCESS,
  ADD_SLOTHOLIDAY_FAILURE,
} from "./actionTypes";

export const getAllSlotholiday = (q,limit,page) => async (dispatch) => {
//console.log('page1234',query1,typeof show);
const token=localStorage.getItem("token")
//const limit=Number(show)
  try {
    dispatch({ type: GET_ALL_SLOTHOLIDAY_REQUEST });
   
    
    const res=  await fetch(`http://localhost:8080/slotholiday/getall?query=${q}&limit=${limit}&page=${page}`, {
...
console.log('slotholiy',data);
    dispatch({ type: GET_ALL_SLOTHOLIDAY_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: GET_ALL_SLOTHOLIDAY_FAILURE,
      payload:error
    });
  }
};
...
export const getAllSlotholiday2 = (q) => async (dispatch) => {
  //console.log('page1234',query1,typeof show);
  const token=localStorage.getItem("token")
  //const limit=Number(show)
    try {
      ...
      const res=  await fetch(`http://localhost:8080/slotholiday/getall?${q}`, {
        // body: JSON.stringify(res),
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
export const deleteslotholidaydata = (id) => async (dispatch) => {
  try {
    dispatch({ type: DELETE_SLOTHOLIDAY_REQUEST });

    await fetch(`http://localhost:8080/slotholiday/${id}`, {
      method: "DELETE",
      headers: {
       // token: localStorage.getItem("token"),
        "Content-Type": "application/json",
    ...
    dispatch({ type: DELETE_SLOTHOLIDAY_SUCCESS, payload: id });
  } catch (error) {
    dispatch({
      type: DELETE_SLOTHOLIDAY_FAILURE,
    });
  }
};


// export const deleteslotholidaydata2 = (date,time) => async (dispatch) => {
//   try {
//     dispatch({ type: DELETE_SLOTHOLIDAY_REQUEST });

//     await fetch(`http://localhost:8080/slotholiday/datetime`, {
//       method: "DELETE",
//       headers: {
//        // token: localStorage.getItem("token"),
//         "Content-Type": "application/json",
...
export const addslotholidayetails = (data) => async (dispatch) => {
  console.log('datajhbhb',data);
    try {
      dispatch({ type: ADD_SLOTHOLIDAY_REQUEST });
      ...
      axios.post('http://localhost:8080/slotholiday/upload',
          data,
          // {
          ...
          .then((res) => {
              console.log(res.data)
              dispatch({ type: ADD_SLOTHOLIDAY_SUCCESS, payload: res.data });
