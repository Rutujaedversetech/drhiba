...
//const limit=Number(show)
  try {
    dispatch({ type: GET_ALL_SLIDEIMAGE_REQUEST });
   
    
    const res=  await fetch(`http://localhost:8080/slideshow/getall`, {
...
console.log('fgh12345',data);
    dispatch({ type: GET_ALL_SLIDEIMAGE_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: GET_ALL_SLIDEIMAGE_FAILURE,
      payload:error
    });
  }
};
...
export const deleteSlideimage = (id) => async (dispatch) => {
  try {
    dispatch({ type: DELETE_SLIDEIMAGE_REQUEST });

    await fetch(`http://localhost:8080/slideshow/${id}`, {
      method: "DELETE",
      headers: {
       // token: localStorage.getItem("token"),
        "Content-Type": "application/json",
    ...
    dispatch({ type: DELETE_SLIDEIMAGE_SUCCESS, payload: id });
  } catch (error) {
    dispatch({
      type: DELETE_SLIDEIMAGE_FAILURE,
    });
  }
};
...
export const addSlideShow = (data) => async (dispatch) => {
  console.log('datajhbhb',data);
    try {
      dispatch({ type: ADD_SLIDEIMAGE_REQUEST });
      ...
      axios.post('http://localhost:8080/slideshow/upload',
          ...
          .then((res) => {
              ...
              dispatch({ type: ADD_SLIDEIMAGE_SUCCESS, payload: res.data });
